...
contract Chublins is Ownable, ERC721A, ReentrancyGuard {
    ...
    function makeSVG(chubData memory chub) internal pure returns (string memory) {
        ...
        parts[6] = "'/><g id='chub' filter='url(#"; 
        parts[7] = chub.filterId; 
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        if(_returnPNG[tokenId] == true && _pngsAvailable == true) {
            ...
            output = string(abi.encodePacked(_baseTokenURI, toString(tokenId), '.png'));
        }
        else {
            output = makeSVG(chub);
    ...
    function togglePNGsAvailable() public onlyOwner returns(string memory) {
        if(_pngsAvailable == true) {
            ...
            return("Indicating that PNGs are not available for the collection");
        }
        _pngsAvailable = true;
        return("Indicating that PNGs are available for the collection");
    }
    ...
    function raiseLicenseLevel(uint256 tokenId, uint256 level) public returns(string memory){
        require(ownerOf(tokenId) == msg.sender, "Only the owner of this token can perform this action");
